...
RFM69_REG_FDEV_MSB 			= 0x05 
RFM69_REG_FDEV_LSB 			= 0x06 
RFM69_REG_FRF_MSB 			= 0x07 
RFM69_REG_FRF_MID 			= 0x08 
RFM69_REG_FRF_LSB 			= 0x09 
RFM69_REG_AFC_CTRL			= 0x0B 
RFM69_REG_VERSION 			= 0x10   #Version and serial number
RFM69_REG_PA_LEVEL 			= 0x11 
RFM69_REG_PA_RAMP 			= 0x12 
RFM69_REG_OCP 				= 0x13 
RFM69_REG_LNA 				= 0x18 
RFM69_REG_RX_BW 			= 0x19 
RFM69_REG_AFC_FEI 			= 0x1E 
RFM69_REG_AFC_BW 			= 0x1A 
RFM69_REG_RSSI_CONFIG		= 0x23 
RFM69_REG_RSSI_VALUE 		= 0x24 
RFM69_REG_DIO_MAPPING1 		= 0x25 
RFM69_REG_DIO_MAPPING2 		= 0x26 
RFM69_REG_IRQ_FLAGS1 		= 0x27 
RFM69_REG_IRQ_FLAGS2 		= 0x28 
RFM69_REG_PREAMBLE_MSB 		= 0x2C 
RFM69_REG_PREAMBLE_LSB 		= 0x2D 
RFM69_REG_SYNC_CONFIG		= 0x2E 
RFM69_REG_SYNC_VALUE1 		= 0x2F 
RFM69_REG_SYNC_VALUE2 		= 0x30 
RFM69_REG_PACKET_CONFIG1 	= 0x37 
RFM69_REG_PAYLOAD_LENGTH 	= 0x38 
RFM69_REG_FIFO_THRESHOLD 	= 0x3C 
RFM69_REG_PACKET_CONFIG2 	= 0x3D 
RFM69_REG_AES_KEY1 			= 0x3E
RFM69_REG_TEST_DAGC			= 0x6F 
...
RFM69_REG_TEST_PA1			 	 = 0x5A 
RFM69_REG_TEST_PA2			 	 = 0x5C 
RF_RXBW_EXP_5					 = 0x05 
...
RF_TEST_PA1_BOOST = 0x5D
RF_TEST_PA2_BOOST = 0x7C
...
def check_timeout(flag, limit: float) -> bool:
	"""Return True if `limit` seconds elapsed before `flag` became true."""
...
class RFM69:
	def __init__(
		...
		*,
		...
		try:
			...
			raise RuntimeError(
				f"SPI ontbreekt: {dev_path} bestaat niet. Zet SPI aan (sudo raspi-config → "
				f"Interface Options → SPI → Yes), of voeg toe aan /boot/firmware/config.txt: "
				f"dtparam=spi=on — daarna reboot. Check: ls /dev/spidev*"
			) from e
		except PermissionError as e:
			raise RuntimeError(
				f"Geen toegang tot {dev_path}. Voeg je gebruiker toe aan groep spi: "
				f"sudo usermod -aG spi,gpio $USER — log uit en weer in (of reboot)."
			) from e
	...
	def reset(self) -> None:
		"""Hardware reset sequence (same timing as MCHobby MicroPython driver)."""
		self._reset.off()
		time.sleep(0.0001)
		self._reset.on()
		time.sleep(0.0001)
	...
	def crc_auto_clear_off( self, value ):
		reg = self.spi_read( RFM69_REG_PACKET_CONFIG1 ) & 0b11110111
		self.spi_write( RFM69_REG_PACKET_CONFIG1, reg | (value<<3) )
	...
	def spi_read(self, register: int) -> int:
		resp = self._spi.xfer2([register & ~0x80, 0])
		return resp[1]

	def spi_burst_read(self, register: int, length: int) -> bytes:
		tx = [register & ~0x80] + [0] * length
		rx = self._spi.xfer2(tx)
		return bytes(rx[1:])

	def spi_write(self, register: int, value: Union[int, bytes, bytearray]) -> None:
		if isinstance(value, (bytes, bytearray)):
			self._spi.xfer2([register | 0x80] + list(value))
		else:
			self._spi.xfer2([register | 0x80, value])

	def spi_write_fifo(self, data: Union[bytes, bytearray]) -> None:
		payload = bytes([RFM69_REG_FIFO | 0x80, len(data)]) + bytes(data)
